...
'''核心更改就是G的计算方式，之前是先计算所有的G，再计算损失函数，现在是在逆序遍历中直接计算G并更新损失函数，这样可以避免在第二次遍历时G已经被更新了的问题。'''
...
class Agent:
    ...
    def updata(self):
        G,loss = 0,0
        #这里是使用REINFORCE算法的核心部分，计算回报和损失函数，使用的回报是从当前时间步开始的未来奖励的折扣和。我们逆序遍历记忆中的奖励和动作概率，计算每个时间步的回报，并将其乘以对应动作概率的对数，累加到损失函数中。最后，我们对损失函数进行反向传播，更新策略网络的参数。
        for reward, prob in reversed(self.memory):
            G = reward + self.gamma * G # 计算回报
            loss += -G * prob.log() # 直接在逆序遍历中计算损失函数，负号是因为我们要最大化回报
